...
void differentialFunction(float angle)
{
  ...
  if (abs(angle) > 2)
  {                                                              // a little tolerance for steering wheel angle
  ...
  */
  ...
  if (differential[0] > 0)
    differential[0] = 0; // prevent speeding up the left wheel
  if (differential[1] > 0)
  ...
  if (throttleLeftValue < 0)
    throttleLeftValue = 0;

  if (throttleRightValue < 0)
  ...
  Serial.println(scaledThrottleLeft);

  Serial.print(" | Right Out: ");
  Serial.println(scaledThrottleRight);
}
